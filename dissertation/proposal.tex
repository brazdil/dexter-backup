...
way. TaintDroid integrates into the lower levels of the operating 
system, altering Dalvik's memory management and instructions to store 
and propagate the taint transparently to the applications running on top
of it. However, similar result can be accomplished by extracting the 
executable code from the application's package and instrumenting it to 
carry out the information-flow analysis itself, without any 
modifications to the platform needed, which is what this project will try 
to attain.
...
Dynamic data-flow analysis will form the core of the project, but based
...
Downside of this solution is that the taint-propagation rules need to
be rather conservative, leading to false positives. This is the reason
...
separately, the user will be given the choice of instrumentation
