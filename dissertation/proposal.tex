...
static analysis of the executable code, all the way to 
...
it can do whatever it wants with it, not needing any further consent from 
the user.
...
It adds support for runtime labelling (tainting) of sensitive data like 
the phone number, the contact list or GPS location, and traces the flow 
...
the system by patching the IPC kernel module, or storing taint within 
...
a software-verification tool for JVM developed by NASA. My plugin dynamically 
verifies correct usage of physical units in scientific computation by 
runtime assignment of attributes with unit information to numerical values 
in the memory, and their propagation during arithmetic operations, which is 
an idea very similar to taint-based flow tracing. 
...
learning more about the possibilities of static and dynamic analysis
of bytecode.
