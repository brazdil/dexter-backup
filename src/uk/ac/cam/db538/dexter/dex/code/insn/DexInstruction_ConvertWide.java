...
public class DexInstruction_ConvertWide extends DexInstruction {
  ...
  protected DexCodeElement gcReplaceWithTemporaries(Map<DexRegister, DexRegister> mapping) {
    return new DexInstruction_ConvertWide(getMethodCode(), mapping.get(regTo1), mapping.get(regTo2), mapping.get(regFrom1), mapping.get(regFrom2), insnOpcode);
  }
  ...
  public Instruction[] assembleBytecode(DexCode_AssemblingState state) {
    ...
    if (fitsIntoBits_Unsigned(rTo1, 4) && fitsIntoBits_Unsigned(rFrom1, 4))
      return new Instruction[] { new Instruction12x(Opcode_ConvertWide.convert(insnOpcode), (byte) rTo1, (byte) rFrom1) };
    else
